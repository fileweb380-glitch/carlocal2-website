...
const links = ["home", "cars", "about", "contact"];
 function Navbar({ active }) {
  ...
  const scrollTo = (id) => {
   const el = document.getElementById(id)?.scrollIntoView({ behavior: "smooth" });
    if (el) {
    ...
    <div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }}
          ...
          <div style={{
            ...
            fontWeight: 900, fontSize: 16, color: "#0a0a0a", letterSpacing: 1
          }}>N</div>
          <span style={{ fontSize: 22, fontWeight: 800, letterSpacing: 3,
          ...
          {links.map((l) => (
            <li key={l}>
              <button onClick={() => scrollTo(l)} style={{
                ...
                onMouseLeave={e => e.target.style.color = active === l ? "#c8a96e" : "#aaa"}
              >{l}</button>
            </li>
          ))}
        </ul>
        ...
        <button onClick={() => scrollTo("contact")} style={{
          ...
          onMouseEnter={e => { e.target.style.transform = "scale(1.05)"; e.target.style.boxShadow = "0 0 20px rgba(200,169,110,0.4)"; }}
        ...
        <button onClick={() => setMenuOpen(!menuOpen)} style={{
          ...
          cursor: "pointer", color: "#c8a96e", fontSize: 24
        }} id="hamburger">☰</button>
      </nav>
      ...
      {/* Mobile Menu */}{menuOpen && (
        <div style={{
          ...
          background: "rgba(10,10,10,0.97)", zIndex: 999,
          ...
          {links.map((l) => (
            <button key={l} onClick={() => scrollTo(l)} style={{
              ...
              textTransform: "uppercase", color: active === l ? "#c8a96e" : "#f5f5f5",
              fontFamily: "Inter, sans-serif"
            }}>{l}</button>
          ))}
        </div>
